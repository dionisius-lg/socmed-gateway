...
/**
 * Get instagram comment details by comment ID
...
exports.getCommentById = async (req, res) => {
    ...
    try {
        const endpoint = `/instagram/comments?page_access_token=${query.account_token}&comment_id=${query.comment_id}`;
        ...
        loggerHelper.error({
            source: 'instagram-comment:getCommentById',
            message,
            error: err?.cause
        });
...
/**
 * Send text comment on a instagram post
 * @param {import('express').Request} req - Express request
 * @param {import('express').Response} res - Express response
 */
exports.sendCommentText = async (req, res) => {
    ...
    try {
        const endpoint = `/instagram/comments/send?page_access_token=${body.account_token}`;
        ...
        loggerHelper.error({
            source: 'instagram-comment:sendCommentText',
            message,
            error: err?.cause
        });
...
/**
 * Send text comment reply to comment on a instagram post
 * @param {import('express').Request} req - Express request
 * @param {import('express').Response} res - Express response
 */
exports.sendCommentTextReply = async (req, res) => {
    ...
    try {
        const endpoint = `/instagram/comments_reply/send?page_access_token=${body.account_token}`;
        const api = await apiHelper.post(endpoint, {
        ...
        loggerHelper.error({
            source: 'instagram-comment:sendCommentTextReply',
            message,
            error: err?.cause
        });
